...
namespace Network
{
	...
	public:
		...
		virtual void execute(GameStateRewinder& gameStateRewinder, WorldHolder& worldHolder) const = 0;
		[[nodiscard]] virtual Ptr clone() const = 0;
		virtual void serverSerialize(WorldHolder& worldHolder, std::vector<std::byte>& inOutStream, ConnectionId receiverConnectionId) const = 0;
	};
